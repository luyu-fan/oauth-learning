...
让第三方客户端访问受保护的资源是整个OAuth设计的初衷，在获取到访问token之后，便可以访问那些受保护的资源。资源服务器首先会验证访问token确保它没有过期，另外访问的范围也是正确的。

#### 访问token类型

访问类型提供给客户端来实际使用这个token的必要信息。不同的token类型导致的格式以及请求都不相同，如果客户度不能很好理解和解析访问类型，那么他就不能使用这个token。

一般常见的有`Bearer`、`Basic`和`MAC`等不同类型的Token。各自的验证授权方式以及安全性都有一定的不同。

每种访问token类型都定义了和访问token一起发送给客户端的额外出行。它同样定义了在利用访问token获取受保护资源时的客户端验证方法。

#### 错误响应

如果资源请求失败了，资源服务器同样也要通知客户端这个错误，至于是什么错误则由资源服务器的服务文档定义而定。

如果资源服务器想提供新的授权验证方式，那么它首先得定义为客户端提供错误信息的这样一种机制。如果错误值是以一个命名的参数返回的，那么这个参数只能是`error`。

其他的关键信息也是如此，都需要指明对应的字段以返回额外的错误信息。

## 可扩展性

可扩展性指的就是可以自定义访问类型以及终端参数。

### 定义访问token类型

访问token类型可以通过两种方式定义，在访问类型注册中心直接注册。或者使用一个唯一的绝对的URI。这个主要值得是获取资源时的客户端请求类型，比较偏向身份验证和请求的组织格式。

### 定义新的终端参数

新的请求或者响应参数也可以在注册中心进行定义，以满足验证或者获取token时的一些需求。

### 定义新的授权类型

新的授权类型可以通过给`grant_type`参数赋予一个独一无二的绝对URI值，显然如果需要额外的参数，那么这些参数也应该在注册中心进行注册。

### 定义新的授权终端响应类型

同样，新的响应类型在授权终端也可以提前定义和注册，但是也需要满足一定的格式要求。如果一个响应类型包含一个或者多个空格隔开的值，虽然这些值出现的顺序不重要，但是只有一种确定的顺序可以被注册，他可以覆盖所有的组合。

即只能有一种确定的组合顺序可以注册，但是这些值的任意组合都可以作为值来代表这一种类型。

### 定义额外的错误码

以防额外定义的授权类型或者是协议需要额外的错误码，这些相关联的错误码也需要提前定义和注册。错误码必须满足一定的格式并且应该尽可能满足前缀的特点。

## 本地应用

原生应用指的是那些运行在资源拥有者的设备上的一些客户端，例如桌面应用或者是移动端应用。原生应用必须考虑到安全性、平台兼容性以及用户体验等。

授权终端一般都会要求客户端上资源拥有者与授权服务器之间有一个交互过程存在，因此原生应用可以考虑提供一个内嵌的用户代理或者是能够唤起一个用户代理。

- 外部的用户代理： 原生应用可以在操作系统唤醒客户端作为handler的情况下，利用授权服务器返回的重定向URI来获取响应。即在本地运行web server 手动复制粘贴身份验证信息，总之需要借助外部工具以及手动参与。

- 嵌入式用户代理：原生应用通过直接和授权服务器交互来监视状态的变化，或者是访问cookie存储。

至于让用户采用外部代理或者是直接内部嵌入的代理，也是在便捷性和安全性做一个权衡。

当选择授权码类型以及隐式类型时候，需要考虑以下几个方面：

- 原生应用在使用授权码类型时候不需要使用客户端的身份验证，原因是原生应用无法保存客户端的身份验证信息。
- 当使用隐式授权的时候，刷新token并不会被返回，因此在访问token到期之后还需要再进行一次授权操作。

## 安全性考虑

OAuth框架本身提供了比较好的灵活性，但是也因此考了各种授权方式的安全性也有一定的挑战。主要包含了以下三种类型的客户端： web应用，基于用户代理的应用以及原生应用。

### 客户端身份验证

授权服务器会为客户端身份验证建立客户端信任凭证，而且鼓励尽量采用更强的客户端验证方式。授权服务器应该不给原生应用或者是基于用户代理的应用客户端颁发密码之类的信任凭证。但是对于那些特定设备上原生应用的安装授权服务器可以办法客户端密码。

当客户端授权不可用的时候，授权服务器应该有其他辅助方法来引导客户端进行相关的动作，例如尽心注册或者是其他能够获取身份信息的服务。同样当授权服务器与未授权的服务器进行交互时必须要考虑进行一种安全的交互实现，并且要限制潜在的身份信息的暴露。

### 客户端冒充

需要考虑可能会有客户端去冒充其他客户端并且获取访问token，因此授权服务器必须时刻要验证客户端的身份，如果因为客户端的原因导致认证失败，授权服务器必须引导当前的客户端进行注册以接收授权响应，或者是直接告知资源拥有者进行授权，授权服务器应该显示地强制资源拥有者进行授权并且提供关于客户端以及请求授权域和生命周期的信息。

授权服务器不应该在没有资源拥有者参与的情况下自动处理重复的授权请求，这样以确保请求是来自原始的客客户端而不是来自冒充者。


### 访问tokens

访问token凭证信息必须透明存储，即仅仅在授权服务器、通过验证的资源服务器之间以及所被办法的客户端之间共享。并且应该有一定的安全协议支持。

当使用隐式授权的时候，访问token是通过URI分段进行传输的，因此它有可能会暴露给未授权的部分。同时授权服务器也必须确保各类token不可能被授权服务器之外的任何对象生成、计算出、猜测和修改。

客户端在获取资源的时候应该尽可能地使用最小的访问域， 同样授权服务器也应该同等地考虑客户端请求时的访问范围。

#### 刷新tokens

刷新token和访问token有着相同的安全要求，而且服务器必须维持刷新token和访问token之间的绑定关系。过期的tokens不会验证通过但是可以被授权服务器保存，当有攻击者存在的时候可以借此区分正常的客户端和冒充者。

#### 授权码

授权码的传输也应该使用一种安全的通信通道， 由于授权码是通过重定向的机制完成，那么他就有可能被用户代理以一种潜在的方式泄漏出去，例如用户代理的历史痕迹以及HTTP引用标头。

身份码的核心作用就是作为一种纯文本身份凭证，用来验证授权者就是资源拥有者。当客户端以来于它自己资源拥有者的身份验证时必须要求使用TLS。

授权码只能是短暂的并且是单次使用的，授权服务器一旦发现有重复的授权码请求尝试，应该立即收回已经授予给这个授权码对应的访问token的所有权限。

如果这个客户端可以被授权，那么在给其颁发访问token之后需要确保每次只有这一个客户端在使用这个token。

#### 授权码重定向URI操纵

当在进行授权码类型的请求时，客户端会通过`redirect_uri `参数来指定一个重定向的地址，如果一个攻击者能够操纵重定向的值，他就能通过授权码机制来操纵授权服务器将资源拥有者重定向到其自身控制的URI下。

攻击者能够在一个合法的客户端上创建一个合法的账号并且初始化整个授权流。当攻击者的用户代理向授权服务器发起授权访问时，攻击者就能够从合法客户端哪里获取到并且替换为受自己控制的重定向URI，然后攻击者就欺骗受害者（资源拥有者）在基于这个伪造的URI下完成一整套授权访问操作。

具体流程是，首先在一个授权服务器上，对来自合法受信的客户端的请求会被授权。当重定向uri被替换之后，受害者就会随着带有授权码的响应重定向到攻击者控制的终端上，然后就由攻击者利用原始客户端提供的URI以及授权码完成剩余的授权流程。客户端交换了带有授权码以及访问token给攻击者，然后攻击者就可以利用这些token获取受保护的资源。

为了防止此类攻击，授权服务器必须确保用于获取授权码的重定向是和利用授权码交换访问token时提供的重定向URI是一致的。授权服务器必须要求公开的客户端应该提供信任凭证去注册他们的的重定向URI。如果一个重定向URI是由请求中的参数提供的，那么授权服务器必须验证他们是不是和注册的URI相匹配。

#### 资源拥有者密码验证

资源拥有者的密码验证类型主要针对的是比较早期或者是需要迁移的类型，它降低了客户端存储用户信息的风险，但是却无法评估暴露给客户度的特权凭证等级。

这种授权方式显然比其他方式有更大的风险，因为这种携带密码的方式显然是和这种协议的初衷是相违背的。客户端或许会滥用或者泄漏资源拥有者的身份验证信息给攻击者。

另外由于客户端直接持有资源拥有者的身份信息，所以客户端就可以避免授权交互环节，拥有者无法直接控制客户端的访问范围。

由于缺陷很多，应该尽可能地避免这种授权方式。

#### 请求受信

一般地，访问token、刷新token、资源拥有者的密码以及客户端的身份凭证不能被明文传输，授权码也是如此。

`state`和`scope`参数不应该包含客户端以及拥有者的相关敏感信息。

#### 确保终端认证

为了防止中间人攻击，授权服务器必须确保要求任何请求使用TLS服务器验证。客户端必须要验证授权服务器的TLS认证。

#### 身份凭证猜测攻击

授权服务器必须防止攻击者猜测任何token以及授权码和身份信息等来进行攻击。**必须确保被猜中的可能性要小于2^(-128)，即至少要128位数据长度**。而且授权服务器也应该提供其他的方式来保护用户的身份信息。

#### 网络钓鱼攻击

很多类似的协议在实现时都会遇到一个问题，即很有可能遭受到钓鱼攻击，即将重定向到钓鱼网站，诱导用户输入身份信息。那么包括第三方客户端在内的服务提供者都应该有责任去教导用户端区分钓鱼网站，同时提供正确的身份验证信息。服务端和客户端都应该考虑用户操作的安全性。为了避免钓鱼，TLS是一个很基础的安全机制。

#### 跨站点请求伪造

跨站点请求伪造（CSRF）是一种攻击者造成受害者用户受恶意URI（例如图片、链接、重定向等）的引导到受信任的服务器的攻击方法。

跨站伪造攻击允许攻击者注入自己的授权码或者访问token，此时就会导致客户端此时使用的是这个攻击者的受保护资源而不是自身的，例如在存放银行密码信息时就会因为这种攻击而导致信息泄漏。

此时就要求客户端必须实现CSRF保护机制。通常做法就是要求任何发送给重定向终端都包含绑定了请求和用户代理状态的信息，例如使用cookie的哈希值来验证用户代理。客户端在请求授权时应该使用`state`字段来发送这个值。

一旦用户端进行了授权，授权服务器就会将用户代理重定向到绑定在必须`state`参数中的值。绑定值使得客户端能够通过匹配绑定值与用户代理授权状态中的值来验证请求的合法性。在这个值中必须要包括一个不可猜测的值，以及用户的授权状态，必须要保持一个仅仅由用户代理和客户端可访问的定位访问（例如使用同源策略实现）。

一个基于授权服务器授权终端的跨站点伪造攻击能够导致攻击者可以获得用户为一个恶意的客户端授权而且不会引起任何用户的警觉。

授权服务器必须要实现CSRF保护装置来保护其授权终端，并且确保恶意客户端不能在没有资源拥有者相关交互的情况下获得授权。

#### 点击劫持

在点击劫持攻击中，一个攻击者可以注册一个合法的客户端并且伪造一个非法的站点来加载授权页面，它会在其上增加一个透明的iframe， 并且会覆盖一系列重要的buttons，即增加一组透明的控件。当用户点击这个授权之类的按钮时，他实际上在点击的是一个透明的按钮。这就会导致一个攻击者 误导一个资源拥有者来对他的客户端进行授权，而且这是在没有给予终端用户知觉的情况下产生的。

为了防止此类点击劫持事件发生，在终端用户授权时原生应用应该使用外部的浏览器而不是应用内嵌的浏览器。**因为这样对于绝大多数的浏览器都可以通过`x-frame-options`字段来强制避免使用iframes。这个字段就会有两个字段以供选择，一个是`deny`另外一个是`sameorigin`同源策略，通过这个参数来防止使用任何的iframe，或者是使用一个同源的frame**。对于比较老的浏览器，依然可以通过js中的frame-busting技术来达到目的。

#### 代码注入与输入验证

代码注入攻击是指当需要输入数据或者是需要使用外部变量，并且会被应用使用而且改变应用的行为逻辑，此时就会导致有额外的被攻击的风险存在。这样可能会使得攻击者来获取额外的访问权限以获得应用内部的数据，或者造成恶意攻击。

#### 开放重定向

在OAuth框架下，授权服务器、授权终端、以及客户端重定向终端可能会被不恰当地配置，这样就能成为一个开放重定向器。一个所谓的公开重定向器指的就是一个仅仅使用一个参数来自动重定向用户代理到参数所指定的URI的过程，而且不经过任何验证。

公开重定向器可以用作钓鱼攻击，或其他攻击，来通过使用一个相似或受信任目的的URI授权组件使得用户访问恶意王长。另外，如果授权服务器允许客户端仅仅注册URI的一部分，即一种模式，攻击者便可以伪造符合这种模式的URI使得可以通过服务端的验证从而重定向至恶意网站中从而获得授权码或者是访问token。

#### 在隐式模式中冒充资源拥有者以滥用访问token

对于那些使用隐式流的公开客户端，实际上并没有任何方法来决定应该给那个客户端颁发token。

一个资源拥有者或许会通过给一个恶意客户端授予token来代理访问资源，比如钓鱼攻击的后果。一个攻击者也可以通过其他机制来偷取token。然后攻击者可以尝试通过冒充资源拥有者向其他公开的客户端提供访问token。

在隐式流中，攻击者可以很轻易地切换不同的token，即使用给攻击者颁发的token来替代真正的token。服务器和原生应用通信时依赖于访问token来确定客户端的使用者，攻击者可以凭借此来构造一个假的应用并注入任意的偷来的token。

此类攻击可以暴露资源拥有者的信息给攻击者，即伪装的客户端。这也会允许攻击者在合法的客户端上使用和资源拥有者一样的访问权限来获取访问token或者是授权码。

在使用用户代理向客户端授权的过程中（第三方登录设备），任何第三方客户端都不应该在没有任何额外限制的情况下使用隐式流类型，而是应该提供可以让客户端决定这个访问token是否就是为他的使用所颁发的。

## 其他

### OAuth 访问token类型的注册

访问token类型的注册会需要oauth专家组两个星期的评审周期，这个应该是属于扩展的部分，用于完成自定义的访问token类型。

### 注册模版

模版中主要包括了类型名称、额外的token终端响应参数、http身份验证方法、变更控制器以及相关文档等。

### OAuth参数注册

此项主要会完成参数的注册。需要时查阅模版。

### OAuth授权终端响应类型注册

此项主要会完成授权终端的注册。需要时查阅模版。

### OAuth扩展错误码注册

此项主要会完成错误码的注册。需要时查阅模版。
